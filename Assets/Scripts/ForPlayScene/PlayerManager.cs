...
public static class PlayerManager
{
    ...
    */
    ...
    public static void RegisterController(int deviceID)
    {
        // If the controller is already used, return
        if (DeviceIDToPlayerNumber.ContainsKey(deviceID)) return;
        // If we are at max player count we can't register more controllers, return
        if (DeviceIDToPlayerNumber.Keys.Count == MaxPlayerCount) return;
